    ...
    public class SiteDorcelVision : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (searchNodes != null)
            {
                ...
                {
                    string titleNoFormatting = node.SelectSingleNode(".//img")?.GetAttributeValue("alt", "").Trim();
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            if(actorNodes != null)
            {
                foreach(var actor in actorNodes)
                {
                    ...
                    string actorPhotoUrl = actor.SelectSingleNode(".//img")?.GetAttributeValue("data-src", "");
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if(imageNodes != null)
            {
                ...
                {
                    string imageUrl = (img.GetAttributeValue("href", "")).Replace("blur9/", "/");
