        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (searchNodes != null)
            {
                foreach (var node in searchNodes)
                {
                    ...
                    string curId = Helper.Encode(Helper.GetSearchBaseURL(siteNum) + titleNode?.GetAttributeValue("href", "").Trim());
            ...
            if (actorNodes != null)
            {
                ...
                foreach (var actor in actorNodes)
                {
                    ...
                    if(actorHttp.IsOK)
                    {
                        ...
                        actorPhotoUrl = actorPage.SelectSingleNode("//div[@class='pornstar-pic ']/img")?.GetAttributeValue("src", "");
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if(imageNodes != null)
            {
                foreach(var img in imageNodes)
                    images.Add(new RemoteImageInfo { Url = img.GetAttributeValue("content", "") });
            }

            try
            {
                string photoPageUrl = Helper.GetSearchBaseURL(siteNum) + detailsPageElements.SelectSingleNode("//li[@class='photos']/a")?.GetAttributeValue("href", "");
                ...
                if(photoHttp.IsOK)
                {
                    ...
                    if(unlockedPhotos != null)
                    {
                        ...
                        {
                            string imageUrl = photo.GetAttributeValue("href", "");
