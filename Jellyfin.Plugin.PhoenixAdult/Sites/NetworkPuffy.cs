    ...
    public class NetworkPuffy : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (searchNodes != null)
            {
                foreach (var node in searchNodes)
                {
                    ...
                    string subSiteSrc = node.SelectSingleNode(".//img")?.GetAttributeValue("src", "");
                    string subSite = "Wet and Pissy";
                    if (subSiteSrc != null)
                    {
                        if (subSiteSrc.Contains("weliketosuck")) subSite = "We Like To Suck";
                        if (subSiteSrc.Contains("wetandpuffy")) subSite = "Wet and Puffy";
                        if (subSiteSrc.Contains("simplyanal")) subSite = "Simply Anal";
                        if (subSiteSrc.Contains("eurobabefacials")) subSite = "Euro Babe Facials";
                    }
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            if(allSummary != null && tagsSummary != null)
                movie.Overview = allSummary.Replace(tagsSummary, "").Split(new[] { "Show more..." }, StringSplitOptions.None)[0].Trim();
            ...
            if (dateNode != null && DateTime.TryParse(dateNode.InnerText.Replace("Released on:", ""), out var parsedDate))
            {
            ...
            if (actorNodes != null)
            {
                ...
                foreach (var actor in actorNodes)
                {
                    ...
                    if(actorHttp.IsOK)
                    {
                        ...
                        actorPhotoUrl = actorPage.SelectSingleNode("//div/section[1]/div/div[1]/img")?.GetAttributeValue("src", "");
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            string tagline = item.Tags.FirstOrDefault() ?? "";

            string twitterBg = null;
            string cover = sceneUrl.Split(new[] { "-video-" }, StringSplitOptions.None)[1];
            if (tagline == "Wet and Pissy") twitterBg = $"https://media.wetandpissy.com/videos/video-{cover}cover/hd.jpg";
            else if (tagline == "We Like To Suck") twitterBg = $"https://media.weliketosuck.com/videos/video-{cover}cover/hd.jpg";
            else if (tagline == "Wet and Puffy") twitterBg = $"https://media.wetandpuffy.com/videos/video-{cover}cover/hd.jpg";
            else if (tagline == "Simply Anal") twitterBg = $"https://media.simplyanal.com/videos/video-{cover}cover/hd.jpg";
            else if (tagline == "Euro Babe Facials") twitterBg = $"https://media.eurobabefacials.com/videos/video-{cover}cover/hd.jpg";
            if(twitterBg != null)
