...
namespace PhoenixAdult.Sites
{
    ...
    {
        private readonly Dictionary<string, string> headers = new Dictionary<string, string> { { "Referer", "https://www.puba.com/pornstarnetwork/index.php" } };
        private readonly Dictionary<string, string> cookies = new Dictionary<string, string> { { "PHPSESSID", "rvo9ieo5bhoh81knnmu88c3lf3" } };
        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            {
                var http = await HTTP.Request(sceneURL, cancellationToken, this.headers, this.cookies);
                if (http.IsOK)
                {
