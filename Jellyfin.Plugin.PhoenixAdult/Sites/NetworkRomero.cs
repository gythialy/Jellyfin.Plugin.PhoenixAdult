...
namespace PhoenixAdult.Sites
{
    public class NetworkRomero : IProviderBase
    {
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            movie.Name = (detailsPageElements.SelectSingleNode("//meta[@itemprop='name']")?.GetAttributeValue("content", "") ?? detailsPageElements.SelectSingleNode("//h1")?.InnerText)?.Split('|')[0].Split(new[] { "- Free Video" }, StringSplitOptions.None)[0].Trim();
            ...
            if (dateNode != null && DateTime.TryParse(dateNode.GetAttributeValue("content", "").Split('T')[0].Trim(), out var parsedDate))
            {
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if(imageNodes != null)
            {
                ...
                {
                    string imageUrl = img.GetAttributeValue("src", "") ?? img.GetAttributeValue("href", "");
