...
namespace PhoenixAdult.Sites
{
    public class NetworkFullPornNetwork : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            foreach (var sceneUrl in searchResultsUrls)
            {
                ...
                if (httpResult.IsOK)
                {
                    ...
                    string titleNoFormatting = detailsPageElements.SelectSingleNode("//h1[contains(@class, 'title_bar')]")?.InnerText.Split(':').Last().Trim();
            ...
            foreach (var modelUrl in modelResultsUrls)
            {
                ...
                if (httpResult.IsOK)
                {
                    var modelPageElements = await HTML.ElementFromString(httpResult.Content, cancellationToken);
                    ...
                    if (sceneNodes != null)
                    {
                        ...
                        {
                            string sceneLink = sceneNode.SelectSingleNode(".//a[@class='updateimg']")?.GetAttributeValue("href", "");
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            movie.Name = detailsPageElements.SelectSingleNode("//h1[contains(@class, 'title_bar')]")?.InnerText.Split(':').Last().Trim();
            ...
            if(actorNodes != null)
            {
                foreach(var actor in actorNodes)
                {
                    ...
                    string actorLink = actor.GetAttributeValue("href", "");
                    ...
                    if(actorHttp.IsOK)
                    {
                        ...
                        string actorPhotoUrl = actorPage.SelectSingleNode("//img[@alt='model']")?.GetAttributeValue("src0_3x", "");
