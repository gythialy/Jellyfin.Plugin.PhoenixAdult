...
namespace PhoenixAdult.Sites
{
    public class NetworkDirtyFlix : IProviderBase
    {
        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            HtmlNode tourPageElements1 = null;
            if (tourHttpResult1.IsOK)
            {
                tourPageElements1 = HTML.ElementFromString(tourHttpResult1.Content);
            }
            ...
            HtmlNode tourPageElements2 = null;
            if (tourHttpResult2.IsOK)
            {
                tourPageElements2 = HTML.ElementFromString(tourHttpResult2.Content);
            }
            ...
            for (int i = 2; i < sitePages; i++)
            {
                ...
                if (httpResult.IsOK)
                {
                    ...
                    if (searchNodes != null)
                    {
                        ...
                        {
                            var xPathKvp = XPathDB.FirstOrDefault(kvp => kvp.Key.Contains(Helper.GetSearchSiteName(siteNum)));
                            if (xPathKvp.Key == null || xPathKvp.Value == null)
                            {
                                continue;
                            }

                            var xPath = xPathKvp.Value;
                            string titleNoFormatting = Helper.ParseTitle(searchResult.SelectSingleNode(xPath[0])?.InnerText?.Trim() ?? string.Empty, siteNum);
                            ...
                            if (match.Success)
                            {
                                ...
                                {
                                    var dateNode = tourPageElements.SelectSingleNode(".//span[@class='added']");
                                    if (dateNode != null && DateTime.TryParse(dateNode.InnerText.Trim(), out var parsedTourDate))
                                    {
                                        releaseDate = parsedTourDate.ToString("yyyy-MM-dd");
                                    }
                                }
                                else
                                {
                                ...
                                {
                                    ProviderIds = { { Plugin.Instance.Name, $"{curId}|{releaseDate}|{Helper.Encode(searchUrl)}" } },
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            string searchPageUrl = providerIds.Length > 2 ? Helper.Decode(providerIds[2]) : null;
            var (siteKey, sitePages) = SiteDB[Helper.GetSearchSiteName(siteNum)];
