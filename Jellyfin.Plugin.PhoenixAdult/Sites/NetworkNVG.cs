...
namespace PhoenixAdult.Sites
{
    public class NetworkNVG : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            {
                var pageData = await this.GetPageData(siteNum, sceneId.Value, cancellationToken);
                if (pageData != null)
                {
            ...
            else
            {
                foreach (var sceneUrl in searchResults)
                {
                    ...
                    if (httpResult.IsOK)
                    {
                        ...
                        {
                            var pageData = await this.GetPageData(siteNum, int.Parse(videoIdMatch.Value), cancellationToken);
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            var detailsPageElements = await this.GetPageData(siteNum, sceneId, cancellationToken);
