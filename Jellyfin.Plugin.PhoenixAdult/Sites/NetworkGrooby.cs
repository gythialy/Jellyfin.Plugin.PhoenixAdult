    ...
    public class NetworkGrooby : IProviderBase
    {
            ...
            if(actorNodes != null)
            {
                foreach(var actor in actorNodes)
                {
                    ...
                    if(actorHttp.IsOK)
                    {
                        ...
                        actorPhotoUrl = actorPage.SelectSingleNode("//div[@class='model_photo']//img[@id] | //div[@class='model_photo']/img")?.GetAttributeValue("src0_1x", "");
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if(imageNodes != null)
            {
                ...
                {
                    string imageUrl = img.GetAttributeValue("href", "") ?? img.GetAttributeValue("src0_4x", "");
