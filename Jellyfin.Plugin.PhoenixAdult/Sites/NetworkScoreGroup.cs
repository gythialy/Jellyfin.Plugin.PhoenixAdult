...
namespace PhoenixAdult.Sites
{
    public class NetworkScoreGroup : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            var searchPageElements = await this.GetSearchFromForm(searchTitle, siteNum, cancellationToken);
            if (searchPageElements != null)
            {
                ...
                if (searchNodes != null)
                {
                    foreach (var node in searchNodes)
                    {
                        ...
                        string sceneUrl = node.SelectSingleNode(".//a[contains(@class, 'title')]")?.GetAttributeValue("href", string.Empty).Trim().Split('?')[0] ?? string.Empty;
                        string curId = Helper.Encode(sceneUrl);
                        string actors = Helper.Encode(node.SelectSingleNode(".//small[@class='i-model']")?.InnerText ?? string.Empty);
                        string img = Helper.Encode(node.SelectSingleNode(".//img")?.GetAttributeValue("src", string.Empty) ?? string.Empty);
