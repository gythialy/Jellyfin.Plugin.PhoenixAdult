...
namespace PhoenixAdult.Sites
{
    public class NetworkScoreGroup : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if(sceneId != null)
            {
                string actorName = Regex.Replace(searchTitle, @"\s\d.*", "").Replace(' ', '-');
            ...
            if (searchNodes != null)
            {
                foreach (var node in searchNodes)
                {
                    ...
                    string sceneUrl = node.SelectSingleNode(".//a[contains(@class, 'title')]")?.GetAttributeValue("href", "").Trim().Split('?')[0];
                    string curId = Helper.Encode(sceneUrl);
                    string actors = Helper.Encode(node.SelectSingleNode(".//small[@class='i-model']")?.InnerText);
                    string img = Helper.Encode(node.SelectSingleNode(".//img")?.GetAttributeValue("src", ""));
            ...
            string urlId = Helper.GetSearchSearchURL(siteNum).Replace(Helper.GetSearchBaseURL(siteNum), "");
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            movie.Name = title.Replace("Coming Soon:", "").Trim();
            ...
            foreach (var xpath in xpaths)
            {
                ...
                if(imageNodes != null)
                {
                    foreach (var img in imageNodes)
                    {
                        ...
                        string imageUrl = posterMatch.Success ? posterMatch.Groups[1].Value : (img.GetAttributeValue("src", "") ?? img.GetAttributeValue("href", ""));
        ...
        private async Task<HtmlNode> GetSearchFromForm(string query, int[] siteNum, CancellationToken cancellationToken)
        {
            ...
            if(httpResult.IsOK)
                return await HTML.ElementFromString(httpResult.Content, cancellationToken);
            return null;
        }
    }
}
