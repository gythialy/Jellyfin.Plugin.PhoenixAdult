...
namespace PhoenixAdult.Sites
{
    public class SiteData18Scenes : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            string encodedTitle = searchTitle.Replace("'", string.Empty).Replace(",", string.Empty).Replace("& ", string.Empty);
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if (cover != null)
            {
                result.Add(new RemoteImageInfo { Url = cover.GetAttributeValue("href", string.Empty) });
            }

            var movieWrap = detailsPageElements.SelectSingleNode("//div[@id='moviewrap']//@src");
            if (movieWrap != null)
            {
                result.Add(new RemoteImageInfo { Url = movieWrap.GetAttributeValue("src", string.Empty) });
            }
