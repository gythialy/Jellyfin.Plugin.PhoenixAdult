...
namespace PhoenixAdult.Sites
{
    ...
    {
        private static readonly Dictionary<string, Dictionary<int, List<string>>> ActorDb = new Dictionary<string, Dictionary<int, List<string>>>
        {
            {
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            if (actorNodes != null)
            {
                ...
                if (match.Success && int.TryParse(match.Groups[1].Value, out var id) && ActorDb.ContainsKey(siteName) && ActorDb[siteName].ContainsKey(id))
                {
                    foreach (var actorName in ActorDb[siteName][id])
                    {
