    ...
    public class SiteDarkRoomVR : IProviderBase
    {
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            if(descriptionNode != null)
                movie.Overview = descriptionNode.InnerText.Trim().Replace("Read less", "");
            ...
            if(actorNodes != null)
            {
                foreach(var actor in actorNodes)
                {
                    ...
                    if(actorHttp.IsOK)
                    {
                        ...
                        actorPhotoUrl = actorPage.SelectSingleNode("//img[contains(@class, 'pornstar-detail__picture')]")?.GetAttributeValue("src", "");
                    }
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if(imageNodes != null)
            {
                foreach(var img in imageNodes)
                    images.Add(new RemoteImageInfo { Url = img.GetAttributeValue("href", "") });
            }
