...
namespace PhoenixAdult.Sites
{
    ...
    {
        private readonly Dictionary<string, string> cookies = new Dictionary<string, string> { { "nats", "MC4wLjMuNTguMC4wLjAuMC4w" } };
        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            do
            {
                ...
                var httpResult = await HTTP.Request(searchUrl, HttpMethod.Get, cancellationToken, null, this.cookies);
                if (!httpResult.IsOK)
                {
                    break;
                }
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            var httpResult = await HTTP.Request(sceneUrl, HttpMethod.Get, cancellationToken, null, this.cookies);
            if (!httpResult.IsOK)
            {
                return result;
            }
            ...
            if (actorNodes != null)
            {
                ...
                foreach (var actor in actorNodes)
                {
                    ...
                    var actorHttp = await HTTP.Request(actorPageUrl, HttpMethod.Get, cancellationToken, null, this.cookies);
                    if (actorHttp.IsOK)
                    {
