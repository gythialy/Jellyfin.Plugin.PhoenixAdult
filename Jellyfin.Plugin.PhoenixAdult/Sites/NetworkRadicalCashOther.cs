...
namespace PhoenixAdult.Sites
{
    public class NetworkRadicalCashOther : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            var siteXPath = this.GetXPathMap(siteNum);
            ...
            if (httpResult.IsOK)
            {
                ...
                if (searchNodes != null)
                {
                    ...
                    {
                        string sceneUrl = searchResult.SelectSingleNode(siteXPath["searchURL"])?.GetAttributeValue("href", string.Empty).Split('?')[0] ?? string.Empty;
                        ...
                        string titleNoFormatting = searchResult.SelectSingleNode(siteXPath["searchTitle"])?.InnerText.Trim() ?? string.Empty;
                ...
                if (url.Contains("/model/"))
                {
                    ...
                    if (actorHttp.IsOK)
                    {
                        ...
                        if (actorSearchNodes != null)
                        {
                            ...
                            {
                                var actorSceneUrl = searchResult.SelectSingleNode(siteXPath["searchURL"])?.GetAttributeValue("href", string.Empty).Split('?')[0].Replace("dev.", string.Empty) ?? string.Empty;
                                if (string.IsNullOrEmpty(actorSceneUrl))
                                {
                                    continue;
                                }
                ...
                else
                {
                    ...
                    if (sceneHttp.IsOK)
                    {
                        if (!directSearchResults.Contains(sceneURL.Split('?')[0]))
                        {
                            ...
                            string titleNoFormatting = detailsPageElements.SelectSingleNode(siteXPath["title"])?.InnerText.Trim() ?? string.Empty;
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            var summaryNodes = detailsPageElements.SelectNodes(siteXPath["summary"]);
            if (summaryNodes != null)
            {
                foreach (var desc in summaryNodes)
                {
            ...
            if (actorNodes != null)
            {
                ...
                {
                    string actorName = actor.SelectSingleNode(siteXPath["actor"])?.InnerText.Trim() ?? string.Empty;
                    string actorPhotoUrl = actor.SelectSingleNode(siteXPath["actorPhoto"])?.GetAttributeValue("src", string.Empty) ?? string.Empty;
                    if (siteNum[1] >= 10 && siteNum[1] <= 12)
                    {
                        ...
                        if (actorHttp.IsOK)
                        {
                            ...
                            actorPhotoUrl = modelPageElements.SelectSingleNode("//div[@class='model-photo']//@src")?.GetAttributeValue("src", string.Empty) ?? string.Empty;
                        }
                    }
