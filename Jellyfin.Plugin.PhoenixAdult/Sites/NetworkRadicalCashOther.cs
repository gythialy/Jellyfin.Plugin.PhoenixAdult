...
namespace PhoenixAdult.Sites
{
    public class NetworkRadicalCashOther : IProviderBase
    {
        ...
        private RemoteSearchResult SearchResultBuilder(HtmlNode searchResult, int[] siteNum, DateTime? searchDate, string lang, List<string> directSearchResults)
        {
            ...
            string sceneUrl = searchResult.SelectSingleNode(siteXPath["searchURL"])?.GetAttributeValue("href", "").Split('?')[0];
            if (directSearchResults.Contains(sceneUrl)) return null;
        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (httpResult.IsOK)
            {
                ...
                if(searchNodes != null)
                {
                    ...
                    {
                        directSearchResults.Add(searchResult.SelectSingleNode(siteXPath["searchURL"])?.GetAttributeValue("href", "").Split('?')[0]);
            ...
            {
                string url = sceneUrl.Split('?')[0].Replace("dev.", "");
            ...
            foreach (var sceneUrl in searchResults)
            {
                ...
                if (url.Contains("/model/"))
                {
                    ...
                    if(actorHttp.IsOK)
                    {
                        var actorPageElements = await HTML.ElementFromString(actorHttp.Content, cancellationToken);
                        ...
                        if(actorSearchResults != null)
                        {
                            ...
                            {
                                string scene = searchResult.SelectSingleNode(siteXPath["searchURL"])?.GetAttributeValue("href", "").Split('?')[0].Replace("dev.", "");
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            if (siteNum[0] >= 1852 && siteNum[0] <= 1859) movie.AddStudio("Hitzefrei");
            else if (siteNum[0] >= 1861 && siteNum[0] <= 1862) movie.AddStudio("Gonzo Living");
            ...
            if (actorNodes != null)
            {
                ...
                foreach (var actor in actorNodes)
                {
                    ...
                    string actorPhotoUrl = actor.SelectSingleNode(siteXPath["actorPhoto"])?.GetAttributeValue("src", "");
                    if (siteNum[0] >= 1860 && siteNum[0] <= 1862)
                    {
                        var actorHttp = await HTTP.Request(actorPhotoUrl, HttpMethod.Get, cancellationToken);
                        if(actorHttp.IsOK)
                        {
                            var modelPageElements = await HTML.ElementFromString(actorHttp.Content, cancellationToken);
                            actorPhotoUrl = modelPageElements.SelectSingleNode("//div[@class='model-photo']//@src")?.GetAttributeValue("src", "");
                        }
                    }
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if(imageNodes != null)
            {
                ...
                {
                    string imageUrl = img.GetAttributeValue("href", "") ?? img.GetAttributeValue("poster", "");
