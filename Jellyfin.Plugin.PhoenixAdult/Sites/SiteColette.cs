    ...
    public class SiteColette : IProviderBase
    {
            ...
            if (actorNodes != null)
            {
                ...
                foreach (var actor in actorNodes)
                {
                    ...
                    if(actorHttp.IsOK)
                    {
                        ...
                        var interchange = actorPage.SelectSingleNode("//img[@class='info-img']")?.GetAttributeValue("data-interchange", "");
                        if(interchange != null)
                            actorPhotoUrl = interchange.Replace("[", "").Replace("]", "").Replace(", (small)", "").Replace(", (medium)", "").Replace(", (large)", "").Split(',').Last().Trim();
                    }
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if(interchangeNodes != null)
            {
                ...
                {
                    var interchange = img.GetAttributeValue("data-interchange", "");
                    if(interchange != null)
                        images.Add(new RemoteImageInfo { Url = interchange.Replace("[", "").Replace("]", "").Replace(", (small)", "").Replace(", (medium)", "").Replace(", (large)", "").Split(',').Last().Trim() });
                }
            }
