...
namespace PhoenixAdult.Sites
{
    public class NetworkBellesa : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (sceneId != null)
            {
                var scenePageElements = await this.GetJSONfromAPI("videos", $"filter[id]={sceneId}", siteNum, cancellationToken) as JArray;
            ...
            {
                var searchResults = await this.GetJSONfromAPI("search", $"limit=40&order[relevance]=DESC&q={Uri.EscapeDataString(searchTitle)}&providers=bellesa", siteNum, cancellationToken);
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            var detailsPageElements = await this.GetJSONfromAPI("videos", $"filter[id]={sceneId}", siteNum, cancellationToken);
