...
namespace PhoenixAdult.Sites
{
    public class SiteHeavyOnHotties : IProviderBase
    {
        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            foreach (var sceneUrl in searchResults.Distinct())
            {
                try
                {
                    ...
                    if (httpResult.IsOK)
                    {
                        var scenePageElements = await HTML.ElementFromString(httpResult.Content, cancellationToken);
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            movie.AddCollection(new[] { "Heavy on Hotties" });
            ...
            if(actorNodes != null)
            {
                foreach(var actor in actorNodes)
                {
                    ...
                    string actorUrl = actor.GetAttributeValue("href", "");
                    ...
                    if(actorHttp.IsOK)
                    {
                        ...
                        actorPhotoUrl = actorPage.SelectSingleNode("//div[./h1]/img")?.GetAttributeValue("src", "");
