    ...
    public class NetworkAdultEmpireCash : IProviderBase
    {
        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (sceneId != null)
            {
                ...
                var directHttp = await HTTP.Request(directUrl, HttpMethod.Get, cancellationToken, headers: null, cookies: this.cookies);
                if (directHttp.IsOK)
                {
            ...
            var httpResult = await HTTP.Request(searchUrl, HttpMethod.Get, cancellationToken, headers: null, cookies: this.cookies);
            if (!httpResult.IsOK)
            {
                return result;
            }
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            var detailsPageElements = await HTML.ElementFromURL(sceneUrl, cancellationToken, headers: null, cookies: this.cookies);
