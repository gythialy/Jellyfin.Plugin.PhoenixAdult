...
namespace PhoenixAdult.Sites
{
    public class SiteKillergram : IProviderBase
    {
        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (DateTime.TryParse(ExtractDate(doc), out var parsedReleaseDate))
            {
                releaseDate = parsedReleaseDate.ToString("dd MMMM yyyy");
            }
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            if (DateTime.TryParse(ExtractDate(doc), out var parsedDate))
            {
        ...
        {
            var searchResult = doc.SelectSingleNode("//img[@id='episode_001']")?.GetAttributeValue("src", string.Empty) ?? string.Empty;
            var titleMatches = Regex.Match(searchResult.Trim(), @"https://media.killergram.com/models/(?<actress>[\w ]+)/(\k<actress>)_(?<title>[\w ]+)/.*");
            return titleMatches.Success ? titleMatches.Groups["title"].Value : string.Empty;
        }
        ...
        private static string ExtractDate(HtmlNode doc)
        {
            ...
            return searchResult?.InnerText?.Trim() ?? string.Empty;
        }
