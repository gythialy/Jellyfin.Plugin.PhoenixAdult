...
namespace PhoenixAdult.Sites
{
    public class NetworkGammaEntOther : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            string apiKey = await this.GetApiKey(siteNum, cancellationToken);
            if (apiKey == null)
            {
                return result;
            }
            ...
            foreach (var sceneType in new[] { "scenes", "movies" })
            {
                ...
                var searchResults = await this.GetAlgolia(url, $"all_{sceneType}", parameters, Helper.GetSearchBaseURL(siteNum), cancellationToken);
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            var detailsPageElements = (await this.GetAlgolia(url, $"all_{sceneType}", $"filters={sceneIdName}={sceneId}", Helper.GetSearchBaseURL(siteNum), cancellationToken))?.FirstOrDefault();
            ...
            foreach (var actor in detailsPageElements["actors"])
            {
                ...
                var actorData = (await this.GetAlgolia(url, "all_actors", $"filters=actor_id={actor["actor_id"]}", Helper.GetSearchBaseURL(siteNum), cancellationToken))?.FirstOrDefault();
        ...
        {
            string url = $"{Helper.GetSearchBaseURL(siteNum)}{((siteNum[1] == 87) ? "/login" : "/en/login")}";
