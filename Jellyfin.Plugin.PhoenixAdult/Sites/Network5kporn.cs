    ...
    public class Network5kporn : IProviderBase
    {
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            if (actorNodes != null)
            {
                foreach (var actorNode in actorNodes)
                {
                    ...
                    {
                        var actorPageElements = await HTML.ElementFromURL(actorUrl, cancellationToken, this.cookies);
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            for (int i = 1; i <= 2; i++)
            {
                ...
                var photoPageElements = await HTML.ElementFromURL(photoPageUrl, cancellationToken, this.cookies);
