...
namespace PhoenixAdult.Sites
{
    public class NetworkReptyle : IProviderBase
    {
        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            string searchNetwork = this.GetSubNetwork(subSite, "search");
            ...
            {
                var content = await this.GetJSONfromPage(sceneURL, cancellationToken);
                if (content != null)
                {
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            string searchNetwork = this.GetSubNetwork(Helper.GetSearchSiteName(siteNum), "search");
            if (string.IsNullOrEmpty(searchNetwork))
            {
                searchNetwork = "Reptyle";
            }
            ...
            var detailsPageJson = await this.GetJSONfromPage(Helper.GetSearchSearchURL(siteNum) + sceneName, cancellationToken);
            ...
            {
                var fallbackJson = await this.GetJSONfromPage($"https://www.{searchNetworkCleanLower}.com/movies/{sceneName}", cancellationToken);
            ...
            string subSite = this.GetSubSite(detailsSubSite);
            string subNetwork = this.GetSubNetwork(subSite);
            ...
            if (models != null)
            {
                foreach (var model in models)
                {
                    ...
                    if (string.IsNullOrEmpty(actorPhotoURL))
                    {
                        try
                        {
                            var actorData = await this.GetJSONfromPage($"https://www.{searchNetworkCleanLower}.com/models/{actorID}", cancellationToken);
                            if (actorData != null && actorData["modelsContent"] != null && actorData["modelsContent"][actorID] != null)
                            {
                                actorPhotoURL = actorData["modelsContent"][actorID]["img"]?.ToString();
                            }
