    ...
    public class SitePOVR : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (http.IsOK)
            {
                ...
                var searchNodes = doc.DocumentNode.SelectNodes("//div[@class='thumbnail-wrap']/div");
                if (searchNodes == null)
                {
                    return result;
                }
                ...
                {
                    var titleNode = searchResult.SelectSingleNode(".//div//h6[@class='thumbnail__title']");
                    var subSiteNode = searchResult.SelectSingleNode(".//a[contains(@class, 'thumbnail__footer-link')]");
                    if (titleNode == null || subSiteNode == null)
                    {
                        continue;
                    }

                    var titleNoFormatting = titleNode.InnerText;
                    var sceneURL = searchResult.SelectSingleNode(".//a[@class='thumbnail__link']")?.GetAttributeValue("href", string.Empty) ?? string.Empty;
                    var subSite = subSiteNode.InnerText;
