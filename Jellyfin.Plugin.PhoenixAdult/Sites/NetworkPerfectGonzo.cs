    ...
    public class NetworkPerfectGonzo : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (searchNodes != null)
            {
                foreach (var node in searchNodes)
                {
                    ...
                    if (subSiteNode != null)
                    {
                        string src = subSiteNode.GetAttributeValue("src", "");
                        if (src.Contains("allinternal")) subSite = "AllInternal";
                        else if (src.Contains("asstraffic")) subSite = "AssTraffic";
                        else if (src.Contains("givemepink")) subSite = "GiveMePink";
                        else if (src.Contains("primecups")) subSite = "PrimeCups";
                        else if (src.Contains("fistflush")) subSite = "FistFlush";
                        else if (src.Contains("cumforcover")) subSite = "CumForCover";
                        else if (src.Contains("tamedteens")) subSite = "TamedTeens";
                        else if (src.Contains("spermswap")) subSite = "SpermSwap";
                        else if (src.Contains("milfthing")) subSite = "MilfThing";
                        else if (src.Contains("interview")) subSite = "Interview";
                    }
            ...
            if(actorNodes != null)
            {
                foreach(var actor in actorNodes)
                {
                    ...
                    if(actorHttp.IsOK)
                    {
                        ...
                        actorPhotoUrl = actorPage.SelectSingleNode("//div[@class='col-md-8 bigmodelpic']/img")?.GetAttributeValue("src", "");
                    }
        ...
        public async Task<IEnumerable<RemoteImageInfo>> GetImages(int[] siteNum, string[] sceneID, BaseItem item, CancellationToken cancellationToken)
        {
            ...
            if(imageNodes != null)
            {
                ...
                {
                    string imageUrl = img.GetAttributeValue("src", "") ?? img.GetAttributeValue("data-original", "");
