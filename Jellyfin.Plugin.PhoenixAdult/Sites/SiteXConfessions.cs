...
namespace PhoenixAdult.Sites
{
    public class SiteXConfessions : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            var token = await this.GetToken(siteNum, cancellationToken);
            if (token != null)
            {
                var searchResults = await this.GetDatafromAPI(siteNum, searchTitle, token, cancellationToken);
                ...
                var directMatch = await this.GetDatafromAPI(siteNum, slug, token, cancellationToken, false);
                if (directMatch != null)
                {
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            if (token != null)
            {
                var detailsPageElements = await this.GetDatafromAPI(siteNum, sceneID[0], token, cancellationToken, false);
