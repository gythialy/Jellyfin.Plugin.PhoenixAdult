    ...
    public class NetworkCouplesCinema : IProviderBase
    {
        ...
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (sceneId != null)
            {
                ...
                var httpResult = await HTTP.Request(sceneUrl, HttpMethod.Get, cancellationToken, cookies: this.cookies);
                if (httpResult.IsOK)
                {
            ...
            else
            {
                ...
                {
                    hasNextPage = false;
                    var httpResult = await HTTP.Request(searchUrl, HttpMethod.Get, cancellationToken, cookies: this.cookies);
                    if (!httpResult.IsOK)
                    {
                        break;
                    }
